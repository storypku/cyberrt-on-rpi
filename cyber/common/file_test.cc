...
TEST(FileTest, Glob) {
  ...
  // Match one.
  EXPECT_THAT(Glob("/home"), testing::ElementsAre(std::string("/home")));
  EXPECT_THAT(Glob("/r?ot"), testing::ElementsAre(std::string("/root")));
  // Match multiple.
  EXPECT_THAT(
      Glob("/s?*"),
      testing::AllOf(
          testing::Contains(std::string("/sys")),
          testing::Contains(std::string("/sbin"))));
}
